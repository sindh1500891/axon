@smoke @salesforce @session
Feature: Scottsdale account search with saved session (no MFA)
  As a test automation engineer
  I want to reuse a locally saved Salesforce session
  So that I can skip login and MFA on every test run

  Scenario: Open Scottsdale Gun Club account using saved session
